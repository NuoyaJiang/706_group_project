...
chart_trend_rate = alt.Chart(subset).mark_line(point=True).encode(
    x=alt.X('year:O'),
    y=alt.Y("c_new_tsr:Q", title= 'TB Treatment Success Rate (%)', scale=alt.Scale(type='log', domain=[0, 100])),
...
chart_trend_rate_resis = alt.Chart(subset).mark_line(point=True).encode(
    x=alt.X('year:O'),
    y=alt.Y("success_rate_resistant:Q", title= 'TB Treatment Success Rate (%)', scale=alt.Scale(type='log', domain=[0, 100])),
    color=alt.Color('country:N'),
